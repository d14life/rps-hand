...
export abstract class BaseVisionTask extends BaseTask {
  ...
  // Local adaptation: feed remote video to Google's unchanged task workers.
  ...
  private setupPhoneCamera() {
    const controls = document.getElementById('webcam-controls-container');
    if (!controls || !this.video) return;
    const button = document.createElement('button');
    button.textContent = 'Phone camera · QR';
    button.className = this.enableWebcamButton?.className || '';
    button.type = 'button';
    const quality = document.createElement('select');
    quality.id = 'phoneQuality';
    quality.setAttribute('aria-label', 'Phone video quality');
    quality.innerHTML = '<option value="480">480p</option><option value="720" selected>720p</option><option value="1080">1080p</option>';
    const note = document.createElement('span');
    note.textContent = 'Phone sends video. This computer runs Google tracking.';
    controls.append(quality, button, note);
    button.onclick = async () => {
      this.stopCam(true);
      const attempt = ++this.phoneAttempt;
      button.disabled = true;
      const load = (src: string, ready: () => boolean) => ready() ? Promise.resolve() : new Promise<void>((resolve, reject) => {
        const script = document.createElement('script'); script.src = src;
        script.onload = () => resolve(); script.onerror = () => reject(new Error('Camera connection library failed to load.'));
        document.head.append(script);
      });
      try {
        await Promise.all([
          load('https://cdnjs.cloudflare.com/ajax/libs/peerjs/1.5.4/peerjs.min.js', () => !!(window as any).Peer),
          load('https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js', () => !!(window as any).QRCode),
        ]);
        const url = new URL('../hand-demo/video-link.mjs?v=alien15.18', location.href).href;
        const { receivePhone } = await import(/* @vite-ignore */ url);
        if (this.disposed || attempt !== this.phoneAttempt) return;
        this.closePhone = receivePhone(async (stream: MediaStream) => {
          if (this.disposed || attempt !== this.phoneAttempt) { stream.getTracks().forEach(t => t.stop()); return; }
          try {
            this.runningMode = 'VIDEO';
            this.worker?.postMessage({type:'SET_OPTIONS', runningMode:'VIDEO'});
            ...
            this.enableWebcamButton.innerText = 'Disconnect phone';
            this.enableWebcamButton.disabled = false;
            this.updateStatus('Phone video connected · Google tracking on this computer');
            this.predictWebcam();
          } catch (error) { this.stopCam(true); this.updateStatus(`Phone video error: ${error}`); }
        }, (message: string) => this.updateStatus(message), (message: string) => { this.stopCam(true); this.updateStatus(message); });
      } catch (error) { this.updateStatus(`Phone connection error: ${error}`); }
  ...
  protected async enableCam() {
    ...
    try {
      ...
      if (!this.worker || !this.video || this.disposed || attempt !== this.phoneAttempt) {
  ...
  protected stopCam(persistState = true) {
    this.phoneAttempt++;
    const close = this.closePhone; this.closePhone = undefined; close?.();
    if (this.video && this.video.srcObject) {
  ...
  protected async predictWebcam() {
    if (this.disposed || !this.video?.srcObject || this.runningMode !== 'VIDEO') return;
