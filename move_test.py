...
async def run():
    async with async_playwright() as p:
        ...
        if os.environ.get("VIEW"): await pg.add_init_script("localStorage.setItem('move.view2', " + repr(os.environ["VIEW"]) + ")")
        ...
        stats = await pg.evaluate("""() => {
            ...
            return { visible: !!window.handModel?.visible, gesture: document.getElementById('gestureStats').textContent,
                     ...
                     offset: window.dbg?.offset, fov: document.getElementById('viewFovValue').textContent, saved: localStorage.getItem('move.view2') }; }""")
