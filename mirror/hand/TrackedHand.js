...
function volumeMaterial(material, uniforms) {
  material.onBeforeCompile = shader => {
    ...
    if (material.name === 'Original skin') shader.fragmentShader = shader.fragmentShader.replace('#include <map_fragment>', `
      #include <map_fragment>
      float skinLuma=dot(diffuseColor.rgb,vec3(.2126,.7152,.0722));
      vec3 skinVariation=diffuseColor.rgb-vec3(skinLuma);
      diffuseColor.rgb=clamp(vec3(.43,.32,.275)*(.60+2.0*skinLuma)+skinVariation*.20,0.0,1.0);
    `);
    ...
    shader.vertexShader = shader.vertexShader.replace('#include <skinnormal_vertex>', `
      ...
      vec3 affineNormal=vec3(0.0);
      for(int k=0;k<4;k++){int j=int(skinIndex[k]);affineNormal+=skinWeight[k]*handRotate(handReal[j],transpose(inverse(handStretch[j]))*objectNormal);}
      objectNormal=normalize(mix(affineNormal,handRotate(nr,transpose(inverse(ns))*objectNormal),.25));
      #ifdef USE_TANGENT
    ...
    shader.vertexShader = shader.vertexShader.replace('#include <skinning_vertex>', `
      ...
      vec3 affinePosition=vec3(0.0);
      for(int k=0;k<4;k++){int j=int(skinIndex[k]);vec4 qr=handReal[j],qd=handDual[j];
        affinePosition+=skinWeight[k]*(handRotate(qr,handStretch[j]*transformed)+2.0*handMul(qd,vec4(-qr.xyz,qr.w)).xyz);}
      vec3 volumePosition=handRotate(hr,hs*transformed)+2.0*handMul(hd,vec4(-hr.xyz,hr.w)).xyz;
      transformed=mix(affinePosition,volumePosition,.25);`);
  };
  material.customProgramCacheKey = () => 'mirror-skin-volume-v3-'+material.name;
...
export class TrackedHand {
  static async load(base = './hand/') {
    if (!assetPromises.has(base)) assetPromises.set(base, Promise.all([
      new GLTFLoader().loadAsync(base + 'tracked-hand.glb?v=fold-2'),
  ...
  update(points, handedness='Right') {
    ...
    for(let i=0;i<21;i++){
      ...
      if(i!==0&&!TIPS.has(i)) {
        const base = [1,5,9,13,17].includes(i) ? palmRotation : this.posedQ[i-1].clone().multiply(this.restQ[i-1].clone().invert());
        q = new THREE.Quaternion().setFromUnitVectors(restDir.clone().normalize().applyQuaternion(base),targetDir.clone().normalize()).multiply(base).multiply(this.restQ[i]);
      }
