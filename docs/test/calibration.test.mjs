...
import {Calibration} from '../head/Calibration.js';
...
const head={mode:'first',video:{readyState:0},spatial:{latest:fit,seen:0,target:[0,0,0]},calibrate(samples,distance){calibrated++;assert.ok(samples.length>=16);assert.equal(distance,null);}};
...
for(let i=0;i<=30&&setup.stage==='center';i++){head.spatial.seen=i*80;setup.update(i*80);}
assert.equal(setup.stage,'side');assert.equal(calibrated,1);assert.equal(resets,1);
for(let i=0;i<10;i++){head.spatial.seen=3000+i*80;head.spatial.target=[.05,0,0];setup.update(head.spatial.seen);}
setup.next();assert.equal(setup.stage,'depth');
for(let i=0;i<10;i++){head.spatial.seen=4000+i*80;head.spatial.target=[0,0,-.075];setup.update(head.spatial.seen);}
