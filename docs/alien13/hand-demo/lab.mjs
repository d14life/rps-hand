...
import {installCombinedUI} from './combined-ui.mjs?v=alien13';
import {CombinedHead} from './head-model.mjs?v=alien13';
...
const $=id=>document.getElementById(id),clone=x=>JSON.parse(JSON.stringify(x)),RAD=Math.PI/180,KEY='alien13-archive-hand-lines-experimental-v1';
...
function rawCameraPoints(world,lm){
 ...
 if(combined?.group.visible&&combined.depth){
  ...
  depth=Math.min(depth,Math.max(.08,combined.depth-.04-protrusion));
 }
...
try{const saved=JSON.parse(localStorage.getItem('alien13-archive-brother-demo-start')||'null');if(saved)restoreStart(saved);}catch{}
$('saveStart').onclick=()=>{try{localStorage.setItem('alien13-archive-brother-demo-start',JSON.stringify({...Object.fromEntries(startFields.map(id=>[id,+$(id).value])),...Object.fromEntries(booleanStartFields.map(id=>[id,$(id).checked]))}));$('startStatus').textContent='Starting settings saved in this browser.';}catch{$('startStatus').textContent='Browser storage unavailable.';}};
