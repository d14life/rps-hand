...
export class OneHandSweep {
 ...
 update(now,hands,face){
  ...
  return {progress:elapsed/1000,message:'Recording '+(elapsed/1000).toFixed(1)+'/8.0 s — move your open hand back toward your shoulder/neck. One way only. '+count+' frames accepted. '+(reason||(!face?'Face briefly lost; hand recording continues.':elapsed>=7000?'Finish at your neck.':'Tracking hand.'))};
 }
}
