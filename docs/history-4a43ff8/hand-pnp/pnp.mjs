...
// Retain MediaPipe's estimated metric scale; do not normalize to a palm length.
export function worldHand(world){
 if(world?.length!==21||!world.every(p=>p&&['x','y','z'].every(k=>Number.isFinite(p[k]))))return null;
 const origin=world[0];
 return world.map(p=>[p.x-origin.x,p.y-origin.y,p.z-origin.z]);
}
...
export function solvePalmPnP(cv,lm,model,k,aspect,ids=PALM_IDS){
 if(!model||model.length!==ids.length||!ids.every(i=>lm?.[i]&&Number.isFinite(lm[i].x)&&Number.isFinite(lm[i].y))||!k||Math.abs(aspect/k.aspect-1)>.01)return null;
 const w=640,h=w/aspect,im=ids.flatMap(i=>[lm[i].x*w,lm[i].y*h]);
 const obj=cv.matFromArray(ids.length,1,cv.CV_64FC3,model.flat()),img=cv.matFromArray(ids.length,1,cv.CV_64FC2,im),K=cv.matFromArray(3,3,cv.CV_64F,[k.fx*w,0,k.cx*w,0,k.fy*h,k.cy*h,0,0,1]),D=cv.matFromArray(5,1,cv.CV_64F,k.distortion||[0,0,0,0,0]),r=new cv.Mat(),t=new cv.Mat(),projected=new cv.Mat(),R=new cv.Mat();
 try{
  ...
  const depth=t.data64F[2],error=Math.sqrt(im.reduce((s,v,i)=>s+(projected.data64F[i]-v)**2,0)/ids.length);
...
export function pnpSelfTest(cv){
 ...
 // Exercise the production 21-point path without any physical-length input.
 const world=Array.from({length:21},(_,i)=>({x:(i%5-2)*.015,y:Math.floor(i/5)*.02,z:.008*Math.sin(i)}));
 const reference=worldHand(world),ids=world.map((_,i)=>i);
 for(const depth of [.25,.5,.9]){
  const lm=reference.map(([x,y,z])=>({x:k.fx*(x+.02)/(z+depth)+k.cx,y:k.fy*(y-.03)/(z+depth)+k.cy}));
  const result=solvePalmPnP(cv,lm,reference,k,k.aspect,ids);
  if(!result||Math.abs(result.depth-depth)>.001)throw Error('21-point depth recovery failed');
 }
