import {measureVideoFrames} from './video-fps.mjs?v=vda2';
...
export function startTracking(video, onResult, onStats, getOptions=()=>defaults,captureOptions={}) {
 ...
 function startWorker(s){
  ...
  const url=new URL('./tracker.mjs?v=vda2',import.meta.url);url.searchParams.set('task',s.task);url.searchParams.set('delegate',s.delegate);url.searchParams.set('fullBody',s.fullBody?'1':'0');url.searchParams.set('poseModel',s.poseModel||'lite');
