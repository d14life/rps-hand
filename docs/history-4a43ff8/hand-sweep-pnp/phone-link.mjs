import {drawFace,drawShoulders,drawUpperBody,encodeAux,decodeAux} from './face-overlay.mjs?v=sweeppnp1';
import {startTracking,defaults} from './tracking-session.mjs?v=sweeppnp1';
...
import {connectLink,packHands,unpackHands} from '../movement/link.mjs?v=sweeppnp1';
...
export function receivePhone(onResult,onStatus,onEnd,onAux=()=>{},getOptions=()=>defaults,onStats=()=>{}){
 ...
 const url=new URL('camera.html',import.meta.url);url.searchParams.set('pair',id);url.searchParams.set('v','demo9');if(getOptions().fullBody)url.searchParams.set('body','1');if(getOptions().fullBody&&getOptions().handRate>0)url.searchParams.set('connected','1');
