import {calibratedReference} from './reference-scale.mjs?v=sweeppnp1';
import {fitNeckSweep} from './neck-sweep.mjs?v=sweeppnp1';
import {OneHandSweep} from './one-hand-sweep.mjs?v=sweeppnp1';
...
export function installExperiment({container,getHead,getFocal=()=>1,onReference=()=>{},onBegin=()=>{},getNeckReference=()=>null}){
 ...
 panel.innerHTML=`<h2>Front-to-neck sweep · PnP 1</h2>
 ...
 <p>The finish jointly fits both hands and the head, neck and shoulders. OpenCV fits a fixed palm to wrist and base-knuckle pixels, solving rotation and XYZ together. Finger curl changes articulation without entering the root-depth fit. Hand model size and depth change together. The palm surface targets the neck surface without an added gap. The two hands share one fixed scale. The neck reference leaves 30 model cm of space behind it. The phone is assumed to lean back about 10 degrees; this angle is not measured. No extra calibration step.</p>
 <label><input id="useSweepReference" type="checkbox" checked> Use saved sweep calibration</label><label><input id="useShoulderDistance" type="checkbox"> Use camera-to-shoulders distance</label><label>Phone lens to shoulder centre (cm)<input id="shoulderDistance" type="number" min="10" max="250" step="1" value="50"></label><p>Optional measured reference at the finishing pose. Scales hands and bust together without changing their camera alignment. Requires a sweep with visible shoulders. Turning it off restores the original sweep scale.</p>
 ...
 $('touchReset').onclick=()=>{cancel();baseFit=null;active=null;onReference(null);$('touchProgress').value=0;$('touchStatus').textContent='Capture reset. Corrected palm-size depth remains active; no captured rear limit.';};
 for(const id of ['useSweepReference','useShoulderDistance','shoulderDistance'])$(id).onchange=()=>{cancel();active=calibratedReference(baseFit,$('useSweepReference').checked,$('useShoulderDistance').checked,+$('shoulderDistance').value);onReference(active);$('touchStatus').textContent=!baseFit?'Record a sweep to apply this reference.':!active?'Saved sweep disabled; live estimation continues.':$('useShoulderDistance').checked&&!baseFit.shoulderDistance?'Shoulder distance unavailable in this capture. Record again with shoulders visible. Original sweep scale kept.':'Reference applied to both hands and bust. '+(active.distanceReference?'Camera-to-shoulders: '+$('shoulderDistance').value+' cm.':'Original sweep scale.');};
