import {finalConfig,installFinalSettings} from './settings.mjs?v=range1';
...
const installExperiment=installDistanceCalibration;
let mapLab=null;
import {fitPhotoHand} from '../hand-pnp-photo/photo-hand.mjs?v=settings12';
import {fitWholeHand} from '../hand-sweep-neck/whole-hand-placement.mjs?v=touch2.4.18-final';
import {resetHandScale,applyHandScale,translateHand,palmSurface} from '../hand-sweep-neck/calibrated-hand-scale.mjs?v=touch2.4.18-final';
import {rearPlaneCorrection} from '../hand-sweep-neck/neck-sweep.mjs?v=touch2.4.18-final';
import {palmObservation,PalmSweepDepth} from '../hand-sweep-neck/palm-sweep-depth.mjs?v=touch2.4.18-final';
const palmDepth=new PalmSweepDepth();
import {TrackingJitter} from '../hand-sweep-neck/tracking-jitter.mjs?v=touch2.4.18-final';
const trackingJitter=new TrackingJitter();
import {OuterCollision} from '../hand-sweep-neck/outer-collision.mjs?v=touch2.4.18-final';
import {HandContactAssist} from '../hand-sweep-neck/hand-contact-assist.mjs?v=touch2.4.18-final';
import {installExperiment as installLegacyExperiment} from '../hand-sweep-neck/experiment.mjs?v=touch2.4.18-final';
let depthExperiment=null;
import {imagePalmSize,sizeDepth,wallShift} from '../hand-sweep-neck/size-wall-depth.mjs?v=alien18.4';
import {palmSize} from '../hand-sweep-neck/palm-distance.mjs?v=alien13';
import {createRoom} from '../hand-sweep-neck/room.mjs?v=demo9';
import {supportedContact} from '../hand-sweep-neck/surface-contact.mjs?v=demo9';
import {fitHeadGrip} from '../hand-sweep-neck/head-grip.mjs?v=demo9';
import {startTracking as startSweepTracking,defaults as trackingDefaults} from '../hand-sweep-neck/tracking-session.mjs?v=alien18.4';
import {installCombinedUI} from '../hand-sweep-neck/combined-ui.mjs?v=touch2.4.18-final';
import {CombinedHead} from '../hand-sweep-neck/head-model.mjs?v=raw19';
...
import {cameraFrame,cameraUV,cameraPosition,fitPalmDepth,liftCameraLandmarks} from '../hand-sweep-neck/projection.mjs?v=8-final';
import {buildTips,tipWorld,fitPinch,fitThumb} from '../hand-sweep-neck/contact.mjs?v=8-final';
import {FIST,AngleLimiter,alignment,poseAlignment,ClosureTracker,thumbFistWeight,thumbContact,closure,referencePose,Settler,depthEstimate,positionAt,straightJoints,pinchDistance} from '../hand-sweep-neck/motion.mjs?v=8-final';
import {receivePhone} from '../hand-sweep-neck/video-link.mjs?v=shared-phone1';
...
import {FINGERS,JOINTS,blankAngles,emptyProfile,features,matchPose,clampAngles,validateProfile,lockedAxis,constrainJoint,constrainAngles,directionAngles} from '../hand-sweep-neck/profile.mjs?v=8-final';
...
function applyHandInteractions(now){
 ...
 if(!options.nearbyTips&&!options.handsTouch&&!options.outerCollision&&!options.headCollision){contactAssist.reset();if(status.textContent!=='Contact assistance and outer collisions off.')status.textContent='Contact assistance and outer collisions off.';return;}
...
function ensureWorker(){if(workerReady)return workerReady;worker=new Worker(new URL('../hand-sweep-neck/tracker.mjs?v=alien18.4&task=hands&delegate=GPU',import.meta.url),{type:'module'});
...
async function detectAuxFrame(frame,time,task){
 const bitmap=await createImageBitmap(frame),w=new Worker(new URL('../hand-sweep-neck/tracker.mjs?v=alien18.4&task='+task+'&delegate=GPU',import.meta.url),{type:'module'});
 let sent=false;
...
await rig.ready;const {snapshotModel,installSweepSettings}=await import('../hand-sweep-neck/settings.mjs?v=flip22');const restoreModel=snapshotModel(rig);const photoReport=fitPhotoHand(rig,{referencePoints:finalReference.points});extendShellTips(rig,finalReference.shellTipExtraFractions);
...
const fixtures={cheek:'../hand-sweep-neck/references/cheek.jpg',scalp:'../hand-sweep-neck/references/scalp.jpg',forward:'../hand-sweep-neck/references/forward.jpg',seated:'../test/seated_desk.jpg',crossed:'../test/crossed_hands.jpg',gesture:'../test/gesture67.webm'};
...
installFinalSettings($('directSettings'),()=>{drivers.R=directDriver(rig,tips);drivers.L=directDriver(rig,tips);});
$('lockBody').checked=true;$('bothHands').checked=true;$('fingerThickness').value=1;$('tipInset').value=0;
document.querySelector('header span').textContent='Final hand driver · two hands · tracked alien eyes · saved gun grip';
// Keep legacy head/distance controls available without mixing them with final hand controls.
const legacy=document.createElement('details');legacy.id='rangeLegacySettings';legacy.innerHTML='<summary>Head, distance calibration and positioning</summary>';
for(const el of [...$('directSettings').children])if(el.id!=='finalHandSettings'&&el.id!=='rangeCameraPanel')legacy.append(el);
$('directSettings').append(legacy);for(const el of legacy.querySelectorAll('details'))el.style.setProperty('display','block','important');legacy.style.setProperty('display','block','important');
...
// for the inherited event handlers without presenting inactive finger options.
const retained=[];
for(const [title,ids] of [
 ['Head and shoulders',['showHead','lockBody','turnGain','moveGain','neckShare','headSmooth','headSize','headSide','headHeight','headBack','faceOffset','faceGrace','mappedFaceDots']],
 ['Hand positioning',['bothHands','handDistance','handHeight','trackingGrace']],
 ['Tracking performance',['trackerDelegate','cameraFps','handRate','faceRate','shoulderRate','trackingWidth','sceneRate','renderScale','overlayRate','uncappedTracking']]
]){const section=document.createElement('section');section.innerHTML='<h2>'+title+'</h2>';for(const id of ids){const label=$(id)?.closest('label');if(label){section.append(label);label.style.setProperty('display','block','important');}}retained.push(section);}
const distanceSection=$('distanceMethod').closest('section');
retained.push(distanceSection,contactPanel);
for(const el of [...legacy.children])if(el.tagName!=='SUMMARY')el.style.setProperty('display','none','important');
for(const section of retained){legacy.append(section);section.style.setProperty('display','block','important');}
$('dots').checked=false;
$('follow').closest('label').style.display='none';
$('viewReset').textContent='Centre eye view';$('viewReset').onclick=()=>$('mapCentre').click();
$('mode').textContent='FIRST-PERSON RANGE';
const fullscreen=document.createElement('button');fullscreen.textContent='Full screen';fullscreen.onclick=()=>document.fullscreenElement?document.exitFullscreen():document.querySelector('.sceneWrap').requestFullscreen();$('viewReset').after(fullscreen);
$('directSettings').prepend(rangeTabs);rangeTabs.firstElementChild.click();
notice('Final hands + head ready. Connect your camera or iPhone; the range uses the saved gun grip.');
if(location.hostname==='127.0.0.1'&&new URLSearchParams(location.search).has('verify')){
 const {verifyFinalRange}=await import('../.lab-qa/range-verify.mjs?v=1');
 await verifyFinalRange({rig,tips,mapLab,combined,acceptTracking,detect,stopCamera,scene,renderer,getHands:()=>allHands,resetDrivers:()=>{drivers.R=directDriver(rig,tips);drivers.L=directDriver(rig,tips);},setFrame:frame=>{sampleSource=frame;},directDriver,finalConfig});
}
