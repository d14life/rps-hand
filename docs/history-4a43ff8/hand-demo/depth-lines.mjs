 ...
 for(const base of [5,9,13,17]){
  ...
  if(weight<=0){
   ...
   for(let pass=0;pass<4;pass++)for(let joint=1;joint<3;joint++){
    ...
    const w=(1-T.MathUtils.smoothstep(angle,8*Math.PI/180,18*Math.PI/180))*T.MathUtils.smoothstep(Math.min(la,lb)/palm,.05,.12)*T.MathUtils.smoothstep((la+lb)/palm,.22,.4);if(w<=0)continue;
