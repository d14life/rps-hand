...
import {PairedSweep,indexContactSample,fitPairedSweep,pairedSweepDepth} from './paired-sweep.mjs';
...
assert.match(indexContactSample(pairFrame(.4,0).slice(0,1),0).reason,/both hands/);
const separated=pairFrame(.4,0);separated[1].landmarks[8].x+=.2;assert.ok(indexContactSample(separated,0).reason,'do not use a drawn/imagined connection over separated fingertips');
const wrongFinger=pairFrame(.4,0);wrongFinger[1].landmarks[8].x+=.2;wrongFinger[1].landmarks[12]={...wrongFinger[0].landmarks[12]};assert.ok(indexContactSample(wrongFinger,0).reason,'middle contact cannot substitute for index contact');
const stale=pairFrame(.4,0);assert.match(indexContactSample(stale,400).reason,/visible/);
const asyncPair=pairFrame(.4,0);asyncPair[1].time=1;assert.match(indexContactSample(asyncPair,0).reason,/same camera frame/);
...
console.log('PASS: index-only paired capture, one-way timing, unseen-depth recovery, frozen per-hand curves, stale/missing/non-touching rejection');
