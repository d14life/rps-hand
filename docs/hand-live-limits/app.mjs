...
import {directDriver} from './hand-driver.mjs?v=1';
...
const cv=null,drivers={},depthStates={},poseStates={};for(const c of cases){const m=models[c.method==='archive'?'archive':'reference'];drivers[c.id]=directDriver(m.rig,m.tips);depthStates[c.id]=new PalmSweepDepth();}
...
const stateKey=cfg.id+':'+S;if(!drivers[stateKey]){drivers[stateKey]=directDriver(rig,tips);depthStates[stateKey]=new PalmSweepDepth();}
...
const driver=drivers[stateKey],result=driver(pts,S,q,1/15,{noiseDegrees:0,smoothingMs:0,staticInput:false,postCaps:!!cfg.postCaps,postCoupling:cfg.postCoupling||0,lockUpper:!!cfg.lockUpper,baseSplay:!!cfg.baseSplay,upperCoupling:cfg.upperCoupling||0,thickness:cfg.thickness||1,tipInset:0,thumbOpposition:cfg.thumbOpposition||0,fitImage:cfg.method==='rays',experiment:cfg,fitAngles:cfg.method==='angles',lm,width:source.width,height:source.height});
...
rows.push({index,timeMs,id:cfg.id,isPhoto,S,finalAudit:result.finalAudit,fitPoints:result.fitPoints,baseSplayAudit:result.baseSplayAudit,baseBefore:result.baseBefore,baseAfter:result.baseAfter,depthMode,fitError,pnpValid,pnpHeld,palmQuaternion:q.toArray(),palmLength:length,curls,extension,tipGap,observedGap,reprojection,points:p.map(v=>v.toArray()),confidence:detection.handedness[hi][0].score});
...
function reset(){for(const k of Object.keys(drivers))delete drivers[k];for(const k of Object.keys(depthStates))delete depthStates[k];}
