...
export function fitPinch(rig,tips,side,values,basis,limits){const names=['Thumb1','Thumb2','Thumb3','Index1','Index2','Index3'],start=structuredClone(values),work=structuredClone(values),hand=rig.joints[side+'Hand'];
 ...
 const gap=()=>tipWorld(rig,tips,side,'Thumb').distanceToSquared(tipWorld(rig,tips,side,'Index'));
 const score=()=>gap()+names.reduce((s,n)=>s+work[n].reduce((v,a,i)=>v+(a-start[n][i])**2*1e-9,0),0);
 update();let best=score();const initial=Math.sqrt(gap());for(const step of [12,6,3,1.5,.75])for(let pass=0;pass<2;pass++)for(const n of names)for(const axis of (n.endsWith('1')||n==='Thumb2'?[0,1]:[0])){const old=work[n][axis];let winner=old;const l=limits[n][axis],min=l.enabled?l.min:n.startsWith('Thumb')?-100:axis===0?0:-40,max=l.enabled?l.max:n.startsWith('Thumb')?100:axis===0?100:40;for(const sign of [-1,1]){work[n][axis]=Math.max(min,Math.min(max,old+sign*step));update();const next=score();if(next<best){best=next;winner=work[n][axis];}}work[n][axis]=winner;update();}
