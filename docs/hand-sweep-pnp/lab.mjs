...
import {solvePalmPose} from './palm-pnp.mjs?v=sweeppnp1';
let poseCV=null;const poseStates={};
import {fitWholeHand} from './whole-hand-placement.mjs?v=sweeppnp1';
import {resetHandScale,applyHandScale,translateHand,palmSurface} from './calibrated-hand-scale.mjs?v=sweeppnp1';
import {rearPlaneCorrection} from './neck-sweep.mjs?v=sweeppnp1';
import {palmObservation,PalmSweepDepth} from './palm-sweep-depth.mjs?v=sweeppnp1';
const palmDepth=new PalmSweepDepth();
import {TrackingJitter} from './tracking-jitter.mjs?v=sweeppnp1';
const trackingJitter=new TrackingJitter();
import {OuterCollision} from './outer-collision.mjs?v=sweeppnp1';
import {HandContactAssist} from './hand-contact-assist.mjs?v=sweeppnp1';
import {installExperiment} from './experiment.mjs?v=sweeppnp1';
let depthExperiment=null;
import {imagePalmSize,sizeDepth,wallShift} from './size-wall-depth.mjs?v=sweeppnp1';
import {palmSize} from './palm-distance.mjs?v=sweeppnp1';
import {createRoom} from './room.mjs?v=sweeppnp1';
import {supportedContact} from './surface-contact.mjs?v=sweeppnp1';
import {fitHeadGrip} from './head-grip.mjs?v=sweeppnp1';
import {startTracking,defaults as trackingDefaults} from './tracking-session.mjs?v=sweeppnp1';
import {installCombinedUI} from './combined-ui.mjs?v=sweeppnp1';
import {CombinedHead} from './head-model.mjs?v=sweeppnp1';
import {directDriver} from './direct.mjs?v=sweeppnp1';
import {reduceFalseDepthBends} from './depth-lines.mjs?v=sweeppnp1';
import {cameraFrame,cameraUV,cameraPosition,fitPalmDepth,liftCameraLandmarks} from './projection.mjs?v=sweeppnp1';
import {buildTips,tipWorld,fitPinch,fitThumb} from './contact.mjs?v=sweeppnp1';
import {FIST,AngleLimiter,alignment,poseAlignment,ClosureTracker,thumbFistWeight,thumbContact,closure,referencePose,Settler,depthEstimate,positionAt,straightJoints,pinchDistance} from './motion.mjs?v=sweeppnp1';
import {receivePhone} from './video-link.mjs?v=sweeppnp1';
...
import {FINGERS,JOINTS,blankAngles,emptyProfile,features,matchPose,clampAngles,validateProfile,lockedAxis,constrainJoint,constrainAngles,directionAngles} from './profile.mjs?v=sweeppnp1';
const $=id=>document.getElementById(id),clone=x=>JSON.parse(JSON.stringify(x)),RAD=Math.PI/180,KEY='hand-sweep-pnp-profile';
...
function ensureWorker(){if(workerReady)return workerReady;worker=new Worker(new URL('./tracker.mjs?v=sweeppnp1',import.meta.url),{type:'module'});
...
async function detectAuxFrame(frame,time,task){
 const bitmap=await createImageBitmap(frame),w=new Worker(new URL('./tracker.mjs?v=sweeppnp1'+task+'&delegate=GPU',import.meta.url),{type:'module'});
 let sent=false;
...
try{const saved=JSON.parse(localStorage.getItem('hand-sweep-pnp-start')||'null');if(saved)restoreStart(saved);}catch{}
$('saveStart').onclick=()=>{try{localStorage.setItem('hand-sweep-pnp-start',JSON.stringify({...Object.fromEntries(startFields.map(id=>[id,+$(id).value])),...Object.fromEntries(booleanStartFields.map(id=>[id,$(id).checked]))}));$('startStatus').textContent='Starting settings saved in this browser.';}catch{$('startStatus').textContent='Browser storage unavailable.';}};
...
notice('Sweep PnP 1 ready. Record one front-to-neck sweep; tracking runs on the PC.');
