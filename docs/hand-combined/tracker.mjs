...
self.onmessage=async({data})=>{try{
 if(data.type==='init'){
  ...
  const source=data.bitmap||data.image,start=performance.now();try{
   ...
   // All face points are available for the overlay. Only eyes drive expressions; lips do not animate the model.
   if(task==='face')out.face=r.faceLandmarks?.[0]&&r.facialTransformationMatrixes?.[0]?{matrix:Array.from(r.facialTransformationMatrixes[0].data),points:r.faceLandmarks[0].map(p=>({x:round(p.x),y:round(p.y)})),eyes:Object.fromEntries((r.faceBlendshapes?.[0]?.categories||[]).filter(c=>/^eyeLook|^eyeBlink/.test(c.categoryName)).map(c=>[c.categoryName,round(c.score)]))}:null;
