...
export const defaults = {cameraFps:30,handRate:60,faceRate:30,shoulderRate:15,trackingWidth:480,trackerDelegate:'GPU',overlayRate:60,handPriority:false};
export function startTracking(video, onResult, onStats, getOptions=()=>defaults,captureOptions={}) {
 ...
 function startWorker(s){
  ...
  const url=new URL('./tracker.mjs?v=19.6',import.meta.url);url.searchParams.set('task',s.task);url.searchParams.set('delegate',s.delegate);
 ...
 function tick(now,metadata,force=false){
  ...
  for(const s of slots){
   ...
   if(s.task==='pose'&&serial%2)continue;
