...
export const defaults = {cameraFps:30,handRate:60,faceRate:30,shoulderRate:60,trackingWidth:480,trackerDelegate:'GPU',overlayRate:60,handPriority:false};
export function startTracking(video, onResult, onStats, getOptions=()=>defaults,captureOptions={}) {
 ...
 function startWorker(s){
  ...
  const url=new URL('./tracker.mjs?v=19.9',import.meta.url);url.searchParams.set('task',s.task);url.searchParams.set('delegate',s.delegate);
