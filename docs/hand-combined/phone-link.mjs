import {startTracking,defaults} from './tracking-session.mjs?v=18.1';
...
export function receivePhone(onResult,onStatus,onEnd,onAux=()=>{},getOptions=()=>defaults,onStats=()=>{}){
 ...
 const url=new URL('camera.html',import.meta.url);url.searchParams.set('pair',id);url.searchParams.set('v','18.1');
 ...
 connectLink(id,{onStatus:status,onMessage:(kind,payload)=>{
  ...
  try{
   ...
   if(kind==='a'){if(!connected){connected=true;if(box.open)box.close();onStatus('Phone connected: face and shoulders');}onAux(JSON.parse(payloadText(payload)),size);return;}if(kind==='s'){const metrics=JSON.parse(payloadText(payload));onStats(metrics);if(!connected){const errors=Object.values(metrics.errors||{});trackerDetail=errors.length?'Phone tracker error: '+errors.join(' · '):metrics.camera>0?'Phone sending '+metrics.camera+' camera FPS. Initializing tracking…':'Phone camera opened, but no new frames are arriving. Keep its camera page visible.';status(trackerDetail);}return;}if(kind!=='h')return;
