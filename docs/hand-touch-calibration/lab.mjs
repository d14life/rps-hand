import {installVideoReplay} from './video-replay.mjs?v=touch2.4.2';
let videoReplay=null;
import {OuterCollision} from './outer-collision.mjs?v=touch2.4.2';
import {HandContactAssist} from './hand-contact-assist.mjs?v=touch2.4.2';
import {installExperiment} from './experiment.mjs?v=touch2.4.2';
...
import {installCombinedUI} from './combined-ui.mjs?v=touch2.4.2';
import {CombinedHead} from './head-model.mjs?v=touch2.4.2';
import {directDriver} from './direct.mjs?v=touch2.4.2';
import {reduceFalseDepthBends} from './depth-lines.mjs?v=touch2.4.2';
...
function applyHandInteractions(now){
 ...
 const packet=label=>{const h=allHands.find(h=>h.label===label),r=renderedHands[label==='Left'?'L':'R'];return h&&r?{...h,lm:h.landmarks,points:r.result.points.map(p=>p.toArray())}:null;};
...
function rawSizePoints(world,lm){
 ...
 const baseDepth=sizeDepth(size,{size:modelPalmSpan*focal,depth:1},depthStates[side]?.depth||.5),depth=depthExperiment?.depth(side,lm,world,baseDepth,captureAspect)??baseDepth;depthStates[side]={depth};
...
const contactPanel=document.createElement('section');contactPanel.style.display='block';contactPanel.innerHTML='<h2>Optional contact</h2><p>Off by default for camera input. Loading a movement video enables index contact matching. Enable contact matching to close touching finger lines; enable outer collisions to protect the two hands. Hand/head collisions are separate and off by default so moving the head does not push your hands. Finger joints within one hand use bending limits, not self-collision.</p>';
