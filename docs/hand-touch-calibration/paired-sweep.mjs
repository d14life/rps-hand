...
import {solveTouchDepth} from './hand-pair-depth.mjs?v=touch2.4';
import {sweepEndpoints} from './one-hand-sweep.mjs?v=touch2.4';
const median=a=>[...a].sort((a,b)=>a-b)[Math.floor(a.length/2)];
const sub=(a,b)=>a.map((v,i)=>v-b[i]);

// Landmark 12 is the middle fingertip. No nearest-finger switching during capture.
export function middleContactSample(hands,now,aspect=1,manual=0){
 ...
 if(pair.some(h=>![0,5,9,12,13,17].every(i=>inside(h.landmarks[i])))||sizes.some(s=>!s))return {reason:'Keep both palms and middle fingertips inside the image.'};
 const dirs=pair.map(h=>{const a=h.landmarks[9],b=h.landmarks[12],v=[(b.x-a.x)*aspect,b.y-a.y],n=Math.hypot(...v);return n>1e-5?v.map(x=>x/n):null;});
 if(dirs.some(d=>!d)||dirs[0].reduce((s,v,i)=>s+v*dirs[1][i],0)>-.5)return {reason:'Point the two middle fingers toward each other.'};
 const tips=pair.map(h=>h.landmarks[12]),gap=Math.hypot((tips[0].x-tips[1].x)*aspect,tips[0].y-tips[1].y),gapRatio=gap/Math.min(...sizes);
 if(gapRatio>.25)return {reason:'Bring the middle fingertips together. Gap: '+Math.round(gapRatio*100)+'% of palm size.'};
 const [L,R]=pair,fit=solveTouchDepth(L.points[12],R.points[12],L.points[0],R.points[0],{maxSideChange:.015,surfaceGap:0});
 if(!fit)return {reason:'Contact geometry is ambiguous. Keep middle fingers extended and pointing inward.'};
 const values=Object.fromEntries(pair.map((h,i)=>{
  const root=h.points[0],delta=i===0?fit.A:fit.B,depth=-(root[2]+delta[2]);
  return [i===0?'L':'R',{size:sizes[i],depth:depth-manual,ray:root.map(v=>v/-root[2]),offset:sub(h.points[12],root)}];
 }));
...
export class PairedSweep {
 ...
 update(now,hands,face,aspect=1,manual=0){
  ...
  const observed=middleContactSample(hands,now,aspect,manual);
  ...
  if(elapsed<0)return {progress:0,message:'Starting in '+Math.ceil(-elapsed/1000)+' — hold middle fingertips touching near the camera. '+(observed.reason||'Both middle fingertips ready.')};
  if(observed.reason)this.rejected[observed.reason]=(this.rejected[observed.reason]||0)+1;
  return {progress:elapsed/1000,message:'Two-hand sweep '+(elapsed/1000).toFixed(1)+'/8.0 s — move BOTH hands back to the bottom of your neck, keeping middle fingertips touching. '+this.samples.filter(s=>s.elapsed>=0).length+' paired frames. '+(observed.reason||'Contact recorded.')};
 }
}
...
export function fitPairedSweep(samples){
 ...
 return {fit:{kind:'middle-pair',version:1,models,frames:samples.length,residualMm:1000*Math.sqrt(errors.reduce((a,b)=>a+b*b,0)/errors.length)}};
}
