...
export function reduceFalseDepthBends(points,lm,width,height,perspective=true){
 ...
 for(const base of [5,9,13,17]){
  ...
  const weight=(1-T.MathUtils.smoothstep(bend,10*Math.PI/180,20*Math.PI/180))*T.MathUtils.smoothstep(Math.min(...lengths)/palm,.07,.15)*T.MathUtils.smoothstep(screen[base].distanceTo(screen[base+3])/palm,.45,.75);
  if(weight<=0){
   ...
   for(let pass=0;pass<4;pass++)for(let joint=1;joint<3;joint++){
    ...
    const w=(1-T.MathUtils.smoothstep(angle,10*Math.PI/180,20*Math.PI/180))*T.MathUtils.smoothstep(Math.min(la,lb)/palm,.05,.12)*T.MathUtils.smoothstep((la+lb)/palm,.22,.4);if(w<=0)continue;
