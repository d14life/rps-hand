...
import {convexPenetration,forwardClearance} from './convex-contact.mjs?v=touch2.4.3';
...
export class OuterCollision{
 ...
 resolve(head,rig,visible,shift,paired,shape,enabled={head:true,hands:true}){
  ...
  for(let pass=0;pass<4;pass++){
   ...
   if(paired&&sides.length===2&&worldHead.length){const z=forwardClearance([...this.hand.L.map(world),...this.hand.R.map(world)],worldHead);if(z){const delta=new T.Vector3(0,0,z);shift('L',delta);shift('R',delta);headHits++;moved=true;}}
   if(!moved)break;
  }
