...
const refs=[];
const calibrated=installExperiment({container:{prepend(){}},getHead:()=>head,getFocal:()=>1,onReference:fit=>refs.push(fit)});
...
for(now=20000;now<33000;now+=100){
 head.seen=now;const t=Math.max(0,Math.min(1,(now-25000)/6900)),depth=.35+.45*t,centreY=.75-.55*t;
...
timer();assert.equal(calibrated.active,true);assert.match(node('touchStatus').textContent,/Saved for both hands/);assert.equal(refs.length,1);assert.equal(refs[0].kind,'crown-sweep');assert.equal(refs[0].scale,undefined);
