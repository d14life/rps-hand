...
export class OneHandSweep {
 ...
 update(now,hands,face){
  ...
  if(elapsed>=8000){
   ...
   this.cancel();
   return {done:true,progress:8,...(ok?{samples}:{}),message:ok?'Sweep recorded.':'Sweep finished: '+count+' motion frames; start '+early.length+'/3, top of head '+late.length+'/3. '+(reasons||'Missing endpoint observations.')+' Previous capture kept.'};
  }
  ...
  if(elapsed<0)return {message:'Starting in '+Math.ceil(-elapsed/1000)+' — hold one open hand low and near the camera, palm facing you. '+(reason||'Start reference ready. Keep your face visible.'),progress:0};
  if(reason)this.rejected[reason]=(this.rejected[reason]||0)+1;
  const count=this.samples.filter(s=>s.elapsed>=0).length;
  return {progress:elapsed/1000,message:'Recording '+(elapsed/1000).toFixed(1)+'/8.0 s — move your open hand upward and back to the top of your head. One way only. '+count+' frames accepted. '+(reason||(!face?'Face briefly lost; hand recording continues.':elapsed>=7000?'Hold at the top of your head, with the hand still visible.':'Tracking hand.'))};
 }
}
