...
export class OneHandSweep {
 ...
 update(now,hands,face){
  ...
  if(elapsed>=8000){
   const samples=this.samples,early=samples.filter(s=>s.elapsed<500),late=samples.filter(s=>s.elapsed>=7500);
   this.cancel();
  ...
  return {progress:elapsed/1000,message:'Recording '+(elapsed/1000).toFixed(1)+'/8.0 s — move your open hand back toward your shoulder/neck. Continue toward the neck for all eight seconds; do not reverse at four seconds.'};
 }
}
