import {fitNeckSweep} from './neck-sweep.mjs?v=touch2.4.9';
import {OneHandSweep} from './one-hand-sweep.mjs?v=touch2.4.9';
...
export function installExperiment({container,getHead,getFocal=()=>1,onReference=()=>{},onBegin=()=>{}}){
 ...
 panel.innerHTML=`<h2>Front-to-neck sweep · 2.4.9</h2>
 ...
 <p>Press Record and hold still for the <b>5-second countdown</b>. Then move the hand <b>back to your neck for 8 seconds, one way</b>. Finish at the bottom of your neck, keeping the hand and face visible. Hold there for the final second. No upward sweep or return movement.</p>
 ...
 function tick(){
  ...
  if(result.samples){const {fit,error}=fitNeckSweep(result.samples);
   if(fit){active=fit;onReference(fit);$('touchStatus').textContent='Saved for both hands. Neck reference captured with a fixed 10-degree phone-tilt assumption. Hand dimensions stay fixed; rotation does not refit depth. Keep the phone in this position.';}
