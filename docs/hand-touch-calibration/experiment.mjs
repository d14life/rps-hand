...
import {OneHandSweep,sweepEndpoints} from './one-hand-sweep.mjs?v=touch2.3';
...
export function installExperiment({container,getHead,getHandDistance=()=>0}){
 const panel=document.createElement('section');panel.style.display='block';panel.innerHTML=`<h2>One-hand sweep</h2><p>Hold one extended open hand still during the five-second countdown, palm facing yourself. The start is captured automatically. Then move back toward your shoulder/neck for eight seconds, ONE WAY ONLY. Finish at your neck. Keep your face visible at the start and finish. No return sweep or second hand.</p><button id="touchSweep">Record 8-second sweep to neck</button> <button id="touchReset">Reset capture</button><p id="touchStatus" role="status" aria-live="polite">Ready. No capture yet.</p><progress id="touchProgress" max="8" value="0" aria-label="Sweep recording progress" style="width:100%"></progress>`;container.prepend(panel);
 ...
 function tick(){const result=capture.update(performance.now(),latest.hands,latest.face);if(!result)return;$('touchStatus').textContent=result.message;$('touchProgress').value=result.progress;
  if(result.done){cancel();if(result.samples){const fit=fitSweep(result.samples,latest.aspect);if(fit){active=fit;$('touchStatus').textContent='Sweep captured and ACTIVE for both hands. Move naturally. The saved mapping stays unchanged until you record again.';}else $('touchStatus').textContent='Sweep not captured: '+sweepFailure(result.samples,latest.aspect);}}
 }
