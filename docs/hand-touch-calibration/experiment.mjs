...
export function installExperiment({container}){
 ...
 const captureButtons=[find('touchAnchor'),find('touchSweep'),find('touchValidate')];
 ...
 function finish(){
  ...
  if(phase==='sweep'){candidate=fitContact(samples,anchor);status.textContent=candidate?'Sweep complete. Candidate fitted; record independent validation before applying.':'Fit rejected. Repeat measured anchor and sweep.';}
  else{const before=median(samples.map(s=>s.baseline)),after=median(samples.map(s=>error(s,candidate)));if(after<.02&&after<before*.9){active=candidate;status.textContent='Calibration applied. Validation contact error '+(after*1000).toFixed(1)+' mm (uncalibrated '+(before*1000).toFixed(1)+' mm). This checks model consistency, not measured depth accuracy.';}else status.textContent='Validation rejected: '+(after*1000).toFixed(1)+' mm contact error. Base depth retained.';}
 }
 ...
 find('touchAnchor').onclick=()=>begin('anchor');find('touchSweep').onclick=()=>anchor?begin('sweep'):status.textContent='Capture the measured anchor first.';find('touchValidate').onclick=()=>candidate?begin('validate'):status.textContent='Record a calibration sweep first.';
