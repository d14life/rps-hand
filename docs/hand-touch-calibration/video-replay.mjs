...
export function installVideoReplay({container,onStart,onFrame,onView}){
 ...
 const canvas=document.createElement('canvas'),context=canvas.getContext('2d');let frames=[],token=0,url=null,ready=false,workers=[],lastIndex=-1,run=0,checkIndex=null;
 function stop(){checkIndex=null;token++;ready=false;video.pause();workers.forEach(w=>w.close());workers=[];frames=[];lastIndex=-1;play.disabled=restart.disabled=seek.disabled=true;if(url){URL.revokeObjectURL(url);url=null;}video.removeAttribute('src');video.load();}
 ...
 async function load(source){onStart();stop();const id=token;run++;status.textContent='Opening video…';try{
  ...
  status.textContent='Ready: '+frames.length+' frames at 30 samples/s; both hands detected in '+two+'/'+frames.length+'. Use Explore in 3D to inspect contact from the side.';play.textContent='Pause';if(location.hostname==='127.0.0.1'&&new URLSearchParams(location.search).has('reviewVideo')){checkIndex=0;play.textContent='Play';}else await video.play();
