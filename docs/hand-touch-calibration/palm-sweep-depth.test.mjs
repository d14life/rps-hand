...
import {palmObservation,fitPalmSweep,PalmSweepDepth,fitOneHandPalmSweep} from './palm-sweep-depth.mjs';
...
const rows=Array.from({length:40},(_,i)=>{const depth=.22+i*.012;return {depth,observation:pose(depth).o};});
const fit=fitPalmSweep(rows);assert.ok(fit);assert.ok(Math.abs(fit.scale-.075*focal)<1e-12);const snapshot=JSON.stringify(fit),tracker=new PalmSweepDepth();let frame=0,count=0;
...
assert.equal(fitPalmSweep(rows.map(r=>({...r,observation:rows[0].observation}))),null,'stationary capture is not a sweep');
...
const sample=(d,yaw)=>{const {p,lm}=pose(d,yaw);return {points:p,landmarks:lm,focal,face:{neckDepth:.65,handDepth:2}};};
const one=fitOneHandPalmSweep(Array.from({length:5},()=>sample(.25,0)),Array.from({length:5},()=>sample(.65,Math.PI)),aspect);
assert.ok(one,'opposite-facing endpoints must be accepted despite wrong initial raw depth');
assert.ok(Math.abs(one.scale-focal*.075)<1e-10);
assert.equal(fitOneHandPalmSweep([],[],aspect),null);
...
console.log('PASS: one-hand flipped endpoint capture and observation caching');
