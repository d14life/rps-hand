import {directDriver} from './direct.mjs?v=15';
...
function directOptions(lm){return {staticInput:!!sampleSource,confirmDegrees:sampleSource?0:8,contactFraction:.12,noiseDegrees:+$('fingerNoise').value,contactPixels:$('tipContact').checked?8:0,thickness:+$('fingerThickness').value,tipInset:+$('tipInset').value,lm,width:$('preview').width,height:$('preview').height};}
...
stabilityControls.innerHTML='<h2>Tracking stability</h2><label>Remove small finger jiggle (degrees)<input id="fingerNoise" type="range" min="0" max="4" step="0.25" value="1" oninput="this.nextElementSibling.value=this.value"><output>1</output></label><p>This is the one control for ordinary shaking. At 1°, changes smaller than 1° are ignored. Sudden tracking flips above 8° are automatically checked against the next camera measurement.</p>';
$('directSettings').prepend(stabilityControls);
const bodyLink=document.createElement('p');bodyLink.innerHTML='<a class="button" href="../head-shoulders/">Open head + shoulders tracking</a><br><small>One upper-body/head model. Tracks head and shoulders; no hand tracker and no lip animation.</small>';$('directSettings').prepend(bodyLink);
...
// Keep the everyday interface small. The removed shape/continuity controls
// retain safe defaults internally; only the useful depth and stability choices
// remain visible.
...
const hide=element=>element?.style.setProperty('display','none','important');hide($('cameraSettings'));
const headings=[...$('directSettings').querySelectorAll('h2')],shapeHeading=headings.find(h=>h.textContent==='Finger shape & starting settings'),continuityHeading=headings.find(h=>h.textContent==='Tracking continuity');
for(let node=shapeHeading;node&&node!==continuityHeading;node=node.nextElementSibling)hide(node);
for(let node=continuityHeading;node&&node!==scalePanel;node=node.nextElementSibling)hide(node);
