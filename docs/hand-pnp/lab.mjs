...
const $=id=>document.getElementById(id),clone=x=>JSON.parse(JSON.stringify(x)),RAD=Math.PI/180,KEY='hand-pnp-profile';
...
function applyHandInteractions(now){
 ...
 const fit=contactAssist.update({left,right,aspect:captureAspect,now,enabled:options.handsTouch&&!editing,rangePercent:options.touchRange,confirmMs:options.touchConfirm,maxSideChange:options.touchSideLimit/100});
...
try{const saved=JSON.parse(localStorage.getItem('hand-pnp-start')||'null');if(saved)restoreStart(saved);}catch{}
$('saveStart').onclick=()=>{try{localStorage.setItem('hand-pnp-start',JSON.stringify({...Object.fromEntries(startFields.map(id=>[id,+$(id).value])),...Object.fromEntries(booleanStartFields.map(id=>[id,$(id).checked]))}));$('startStatus').textContent='Starting settings saved in this browser.';}catch{$('startStatus').textContent='Browser storage unavailable.';}};
...
notice('Palm PnP ready. Phone sends video; PC runs tracking and depth estimation.');
...
const contactPanel=document.createElement('section');contactPanel.style.display='block';contactPanel.innerHTML='<h2>Optional contact</h2><p>Off by default. Enable contact matching to close touching finger lines; enable outer collisions to protect the two hands. Hand/head collisions are separate and off by default so moving the head does not push your hands. Finger joints within one hand use bending limits, not self-collision.</p>';
contactPanel.append($('handsTouch').closest('label'),$('outerCollision').closest('label'),$('headCollision').closest('label'),handContactStatus);$('pnpPanel').after(contactPanel);
