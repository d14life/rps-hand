...
import {rearPlaneCorrection} from './neck-sweep.mjs?v=photo1';
import {palmObservation,PalmSweepDepth} from './palm-sweep-depth.mjs?v=photo1';
const palmDepth=new PalmSweepDepth();
import {TrackingJitter} from './tracking-jitter.mjs?v=photo1';
...
import {createRoom} from './room.mjs?v=photo1';
import {supportedContact} from './surface-contact.mjs?v=photo1';
import {fitHeadGrip} from './head-grip.mjs?v=photo1';
...
import {FIST,AngleLimiter,alignment,poseAlignment,ClosureTracker,thumbFistWeight,thumbContact,closure,referencePose,Settler,depthEstimate,positionAt,straightJoints,pinchDistance} from './motion.mjs?v=photo1';
import {receivePhone} from './video-link.mjs?v=photo1';
...
import {FINGERS,JOINTS,blankAngles,emptyProfile,features,matchPose,clampAngles,validateProfile,lockedAxis,constrainJoint,constrainAngles,directionAngles} from './profile.mjs?v=photo1';
const $=id=>document.getElementById(id),clone=x=>JSON.parse(JSON.stringify(x)),RAD=Math.PI/180,KEY='hand-pnp-photo-profile';
...
function ensureWorker(){if(workerReady)return workerReady;worker=new Worker(new URL('./tracker.mjs?v=photo1',import.meta.url),{type:'module'});
...
async function detectAuxFrame(frame,time,task){
 const bitmap=await createImageBitmap(frame),w=new Worker(new URL('./tracker.mjs?v=photo1'+task+'&delegate=GPU',import.meta.url),{type:'module'});
 let sent=false;
...
await rig.ready;const photoReport=variant==='archive'?null:fitPhotoHand(rig,{preservePalmRelief:true});notice('Loading OpenCV PnP…');try{poseCV=await loadCV();}catch(e){notice('PnP unavailable: '+e.message);throw e;}
tips=buildTips(rig);
if(rig.photoReference)for(const S of ['R','L'])for(const [f,name]of ['Thumb','Index','Middle','Ring','Pinky'].entries())tips[S+name].copy(rig.photoReference.targets[S][4+4*f]).sub(rig.photoReference.targets[S][3+4*f]);
...
try{const saved=JSON.parse(localStorage.getItem('compare-hand-pnp-photo-'+variant+'-hand-pnp-photo-start')||'null');if(saved)restoreStart(saved);}catch{}
$('saveStart').onclick=()=>{try{localStorage.setItem('compare-hand-pnp-photo-'+variant+'-hand-pnp-photo-start',JSON.stringify({...Object.fromEntries(startFields.map(id=>[id,+$(id).value])),...Object.fromEntries(booleanStartFields.map(id=>[id,$(id).checked]))}));$('startStatus').textContent='Starting settings saved in this browser.';}catch{$('startStatus').textContent='Browser storage unavailable.';}};
...
notice('PnP capture check ready. Capture near, then neck; tracking runs on the PC.');
...
function placePalmBeforeFingers(points,s,q){
 const h=allHands.find(h=>h.label===(s==='L'?'Left':'Right'));if(!h||!poseCV)return points;
...
function applyPalmPlacement(){
 return; // Simple lane: no rear-plane displacement.
...
const pnpStatus=document.createElement('p');pnpStatus.id='pnpStatus';pnpStatus.setAttribute('role','status');pnpStatus.textContent='OpenCV ready. Fixed palm pose fitting; fingers articulate separately. Camera intrinsics are approximate.';$('touchProgress').after(pnpStatus);

for(const id of ['nearbyTips','handsTouch','outerCollision','headCollision','relaxedPalm','reduceShake','tipContact','contact'])if($(id))$(id).checked=false;
for(const id of ['demoContact','demoGrip','headSide','headBack','headHeight','handDistance','handHeight'])if($(id))$(id).value=0;
const simpleCalibration=$('touchSweep').closest('section');
for(const child of [...$('directSettings').children])child.style.setProperty('display','none','important');
...
const photoInfo=document.createElement('p');photoInfo.textContent='Reference lines set up the hand geometry once. Live movement uses tracked finger directions; constraints are optional in Hand tracking. The wrist pivot is inside the palm base. Green lines show the actual model joints; both hands keep fixed proportions.';$('touchSweep').closest('section').prepend(photoInfo);
...
if(!document.body.dataset.gunLab){const {installSweepSettings}=await import('../hand-compare/settings.mjs');installSweepSettings({cameraPanel:firstPerson.panel,version:'PnP',isDistanceRecording:()=>depthExperiment?.recording});document.getElementById('sweep-hand-model').append(photoInfo,photoLink);}

const {installComparison}=await import('../hand-compare/ui.mjs');installComparison({variant,estimator:'PnP'});
