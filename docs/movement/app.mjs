import {HeadLook,bodyDisplacement} from './head-look.mjs?v=17';
import {DustMap} from './map.mjs?v=17';
import {HeadView} from '../head/HeadView.js';
import * as THREE from 'three';
import {setupUI} from './ui.mjs?v=17';
import {SwipeController,measurePointer,selectLeftHand} from './swipe.mjs?v=17';
...
async function start(){
 ...
 try{
  ...
  worker=new Worker(new URL('./tracker.mjs?v=17',import.meta.url),{type:'module'});
  ...
  head=new HeadView({mode:$('headEnabled').checked?'first':'off',interval:100,widths:[384,288,512]});head.pose.sensitivity=1.5;headLook.gain=+$('headGain').value;lastHeadVideo=-1;lastVideo=-1;running=true;lastResult=performance.now();$('start').textContent='Stop camera';status('Show one hand');
