import {ThumbJoystick,measureThumb} from './thumb-joystick.mjs?v=28';
import {setupThumbstick} from './thumbstick.mjs?v=28';

import {HeadLook,bodyDisplacement} from './head-look.mjs?v=28';
import {DustMap} from './map.mjs?v=28';
import {HeadView} from '../head/HeadView.js';
import * as THREE from 'three';
import {setupUI} from './ui.mjs?v=28';
import {SwipeController,measurePointer,selectLeftHand} from './swipe.mjs?v=28';
...
async function start(){
 ...
 try{
  ...
  worker=new Worker(new URL('./tracker.mjs?v=28',import.meta.url),{type:'module'});
...
function controlsChanged(){held.reset();swipe.reset();demoRun=null;trackingUI.clear();$('hint').textContent='Thumb straight = forward; curl it progressively for backward. Fully retracted = backward, including backward-left and backward-right. Left/right keeps the v23 response. Both axes blend for a full circle of movement. A partly bent centred thumb is neutral. Open your hand or tap Rest to stop.';status('Start camera · thumb joystick ready');}
$('movementMode').onchange=()=>{inputMode=$('movementMode').value;stick.reset();held.reset();trackedHand=null;swipe.reset();demo=false;demoRun=null;$('demoControls').classList.remove('visible');$('stickZone').hidden=inputMode!=='touch';$('hint').textContent=inputMode==='touch'?'Drag thumbstick to walk; release to stop.':inputMode==='poses'?'360° thumb joystick: extension/curl controls forward/back; sideways blends diagonals. Fully retracted = backward. Open hand or Rest stops.':'Left index out: move your hand to walk. Bend index to release.';};
