...
function stop(){running=false;busy=false;worker?.terminate();worker=null;stream?.getTracks().forEach(t=>t.stop());stream=null;$('cam').srcObject=null;controller.reset();trackingUI.camera(false);$('start').textContent='Start camera';}
...
function frame(now){
 ...
 if(running){
  if(now-lastResult>220){controller.reset();trackingUI.clear();status('Waiting for tracking');}
