...
export class ThumbJoystick {
 constructor(){this.size=1;this.speed=4.5;this.reset();}
 ...
 receive(sample,time){
  ...
  if(active&&Math.hypot(v.x,v.z)>=Math.hypot(this.x,this.z)&&((this.lastTilt&&distance(t,this.lastTilt)<.035*this.effectiveScale)||change<.07)){this.candidate=null;return;}
  if(!active||change>.8){
 ...
 setSize(value){if(!Number.isFinite(value))return;this.size=Math.max(.65,Math.min(1.6,value));this.needsRest=true;this.stop('SHOW FIST TO RESUME');}
