...
import { connectLink, packHands } from "./link.mjs?v=78";
...
export async function startPhone(code, video, onStatus = () => {}) {
  ...
  const hand = new Worker(new URL("./tracker.mjs?v=78", import.meta.url), { type: "module" });
  const head = new Worker(new URL("./head-tracker.mjs?v=78", import.meta.url), { type: "module" });
  ...
  function setBody(on) {
    ...
    if (bodyW) return;
    try {
      bodyW = new Worker(new URL("../body/worker.mjs?v=78", import.meta.url), { type: "module" });
