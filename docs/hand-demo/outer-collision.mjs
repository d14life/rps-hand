...
import {convexPenetration} from './convex-contact.mjs?v=alien18.8';
...
export class OuterCollision{
 ...
 resolve(head,rig,visible,shift,paired,shape,enabled={head:true,hands:true}){
  ...
  for(let pass=0;pass<4;pass++){
   let moved=false;
   for(const side of sides){const shells=this.hand[side].map(world);let best=null;for(const a of shells)for(const b of worldHead){const hit=convexPenetration(a,b);if(hit&&(!best||hit.depth>best.depth))best=hit;}
    if(best&&best.depth>.00005){const delta=new T.Vector3().fromArray(best.delta);shift(side,delta);if(paired&&sides.length===2)shift(side==='L'?'R':'L',delta);headHits++;moved=true;}}
   if(enabled.hands&&sides.length===2){const L=this.hand.L.map(world),R=this.hand.R.map(world);let best=null;for(const a of L)for(const b of R){const hit=convexPenetration(a,b);if(hit&&(!best||hit.depth>best.depth))best=hit;}
    if(best&&best.depth>.00005){const delta=new T.Vector3().fromArray(best.delta).multiplyScalar(.5);shift('L',delta);shift('R',delta.clone().negate());handHits++;moved=true;}}
   if(!moved)break;
  }
  this.last={head:headHits,hands:handHits};return this.last;
 }
}
