...
export function directDriver(rig,tips){
 ...
 return function(points,side,palmQ,dt,{staticInput=false,confirmDegrees=0,noiseDegrees=1,smoothingMs=0,movementThresholdMm=0,upperCoupling=0,lockUpper=true,contactPixels=0,contactReleasePixels=12,thickness=1,tipInset=0,adjustPoints=null,exactLines=false,thumbTwist=0,lm,width,height}){
  ...
  contact=exactLines?null:contactLatch.update(lm,lm,width,height,contactPixels>0,contactPixels,contactReleasePixels);
  ...
  if(lockUpper&&!exactLines)for(let f=1;f<5;f++){
  ...
  const contactGap=contact&&!exactLines?fitContact(chains[0],chains[contact/4-1],lengths[0],lengths[contact/4-1],hinges.get(contact/4-1)):null;
  for(let f=0;f<5;f++)for(let k=0;k<4;k++)p[1+4*f+k].copy(exactLines?points[1+4*f+k]:chains[f][k]);
  if(adjustPoints&&!exactLines)adjustPoints(p,lm);
  ...
  const shape=side+':'+thickness+':'+tipInset,shapeChanged=exactLines||shape!==lastShape;
  for(let f=0;f<5;f++)for(let k=1;k<=3;k++){
   ...
   if(f===0){const restX=restAcross.clone().addScaledVector(axis,-restAcross.dot(axis)).normalize(),worldX=restAcross.clone().applyQuaternion(palmQ);worldX.addScaledVector(target,-worldX.dot(target));if(worldX.lengthSq()>1e-6){worldX.normalize();const restFrame=new T.Matrix4().makeBasis(restX,axis,new T.Vector3().crossVectors(restX,axis));const worldFrame=new T.Matrix4().makeBasis(worldX,target,new T.Vector3().crossVectors(worldX,target));q.setFromRotationMatrix(worldFrame.multiply(restFrame.invert()));}q.premultiply(new T.Quaternion().setFromAxisAngle(target,thumbTwist*Math.PI/180));}
   if(hinges.has(f)){
