import {findTouchPair,solveTouchDepth} from './hand-pair-depth.mjs?v=alien18.7';
...
import {installCombinedUI} from './combined-ui.mjs?v=alien18.7';
import {CombinedHead} from './head-model.mjs?v=alien18.4';
import {directDriver} from './direct.mjs?v=alien18.7';
...
function keepHandBeforeWall(S,result){
 const hand=rig.joints[S+'Hand'];hand.updateWorldMatrix(true,true);let back=Infinity;
 hand.traverse(mesh=>{if(!mesh.isMesh||!mesh.geometry)return;mesh.geometry.boundingBox||mesh.geometry.computeBoundingBox();wallBox.copy(mesh.geometry.boundingBox).applyMatrix4(mesh.matrixWorld);back=Math.min(back,wallBox.min.z);});
 if(!Number.isFinite(back))return result;
 const shift=wallShift(back,headBackWall());if(shift){const target=hand.getWorldPosition(new THREE.Vector3());target.z+=shift;hand.position.copy(hand.parent.worldToLocal(target));hand.updateWorldMatrix(false,true);for(const p of result.points)p.z+=shift;}
 result.wallClearance=back+shift-lastWallZ;result.wallLimited=shift>0;return result;
}
const relaxedStates={};
...
let headCollider=null;const contactCache={};
function assistHeadContact(S,result,lm){
 const o=getCombinedOptions(),threshold=(o.contactThreshold||0)/100;
 if(!threshold||!combined?.group.visible)return result;
 const bone=combined.bones.head;
 if(!headCollider){
  const inverse=bone.matrixWorld.clone().invert(),positions=[];
  combined.asset.traverse(mesh=>{if(!mesh.isSkinnedMesh)return;const accepted=new Set(combined.contactSamples.filter(s=>s.mesh===mesh&&s.region==='head').map(s=>s.i)),index=mesh.geometry.index,count=index?index.count:mesh.geometry.attributes.position.count;
   for(let i=0;i<count;i+=3){const ids=[0,1,2].map(k=>index?index.getX(i+k):i+k);if(!ids.some(i=>accepted.has(i)))continue;for(const id of ids){const v=mesh.getVertexPosition(id,new THREE.Vector3()).applyMatrix4(mesh.matrixWorld).applyMatrix4(inverse);positions.push(v.x,v.y,v.z);}}
  });
  const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometry.computeBoundingSphere();geometry.computeBoundingBox();headCollider=new THREE.Mesh(geometry,new THREE.MeshBasicMaterial({side:THREE.DoubleSide}));headCollider.matrixAutoUpdate=false;
 }
 headCollider.matrixWorld.copy(bone.matrixWorld);
 const stamp=result.points.map(p=>p.toArray().join(',')).join(';')+bone.matrixWorld.elements.join(',')+':'+o.handDistance+':'+o.handHeight+':'+threshold;
 let delta=contactCache[S]?.lm===lm&&contactCache[S]?.stamp===stamp?contactCache[S].delta:null;
 if(delta===null){
  const bounds=headCollider.geometry.boundingBox.clone().applyMatrix4(bone.matrixWorld),ray=new THREE.Raycaster(),samples=result.points.map(p=>p.clone());
  for(let f=0;f<5;f++)for(let j=0;j<3;j++)samples.push(result.points[1+f*4+j].clone().lerp(result.points[2+f*4+j],.5));
  samples.push(result.points[0].clone().lerp(result.points[9],.5));delta=0;
  for(const p of samples){if(p.x<bounds.min.x||p.x>bounds.max.x||p.y<bounds.min.y||p.y>bounds.max.y)continue;ray.set(new THREE.Vector3(p.x,p.y,bounds.max.z+.1),new THREE.Vector3(0,0,-1));const hit=ray.intersectObject(headCollider,false)[0];if(!hit)continue;
   const surface=hit.point.z+.006, gap=p.z-surface;
   if(gap<=threshold&&p.z>=bounds.min.z-.02)delta=Math.max(delta,surface-p.z);
  }
  contactCache[S]={lm,stamp,delta};
 }
 if(delta>0){const hand=rig.joints[S+'Hand'],p=hand.getWorldPosition(new THREE.Vector3());p.z+=delta;hand.position.copy(hand.parent.worldToLocal(p));hand.updateWorldMatrix(false,true);for(const p of result.points)p.z+=delta;result.wallClearance+=delta;}
 return result;
}
const pairContact={key:null,since:0,A:new THREE.Vector3(),B:new THREE.Vector3()};
function resetPairContact(){pairContact.key=null;pairContact.since=0;pairContact.A.set(0,0,0);pairContact.B.set(0,0,0);}
function shiftRenderedHand(S,delta){const result=renderedHands[S]?.result;if(!result)return;const hand=rig.joints[S+'Hand'],p=hand.getWorldPosition(new THREE.Vector3()).add(delta);hand.position.copy(hand.parent.worldToLocal(p));hand.updateWorldMatrix(false,true);for(const point of result.points)point.add(delta);result.wallClearance+=delta.z;}
function pairLabel(text){const e=$('handPairStatus');if(e&&e.textContent!==text)e.textContent=text;}
function reconcileHands(dt,now){
 ...
 if(editing||!getCombinedOptions().handsTouch||!$('bothHands').checked||!L||!R||L.time!==R.time||L.confidence<.5||R.confidence<.5||!renderedHands.L||!renderedHands.R){resetPairContact();pairLabel(getCombinedOptions().handsTouch?'Hand contact: waiting for two visible hands.':'Hand contact assistance off.');return;}
 ...
 let solution=null;
 if(pair){if(pair.key!==pairContact.key){pairContact.key=pair.key;pairContact.since=now;}if(now-pairContact.since>=120)solution=solveTouchDepth(pair.a,pair.b,left.points[0].toArray(),right.points[0].toArray());}
 else{pairContact.key=null;pairContact.since=now;}
 const alpha=1-Math.exp(-Math.min(dt,.1)/.06);
 pairContact.A.lerp(new THREE.Vector3().fromArray(solution?.A||[0,0,0]),alpha);pairContact.B.lerp(new THREE.Vector3().fromArray(solution?.B||[0,0,0]),alpha);
 // Move complete rigid hands; no finger pose or bone-size changes.
 shiftRenderedHand('L',pairContact.A);shiftRenderedHand('R',pairContact.B);
 const wallLift=Math.max(0,-left.wallClearance,-right.wallClearance);if(wallLift){const lift=new THREE.Vector3(0,0,wallLift);shiftRenderedHand('L',lift);shiftRenderedHand('R',lift);}
 // If either hand needs head clearance, lift BOTH by the same amount to retain contact.
 if(solution&&getCombinedOptions().contactThreshold>0){const beforeL=left.points[0].z,beforeR=right.points[0].z;assistHeadContact('L',left,L.landmarks);assistHeadContact('R',right,R.landmarks);const a=left.points[0].z-beforeL,b=right.points[0].z-beforeR,lift=Math.max(a,b);shiftRenderedHand('L',new THREE.Vector3(0,0,lift-a));shiftRenderedHand('R',new THREE.Vector3(0,0,lift-b));}
 pairLabel(solution?'Hand contact: relative depth aligned.':pair?'Hand contact: checking nearby points.':'Hand contact: hands moving independently.');
 if(solution)$('directStatus').textContent='Hand contact assistance · relative depth corrected';
}
...
function rawSizePoints(world,lm){
 ...
 const depth=sizeDepth(size,{size:modelPalmSpan*focal,depth:1},depthStates[side]?.depth||.5);depthStates[side]={depth};
 ...
 return liftCameraLandmarks(lm,world,captureAspect,camera.aspect,Math.max(.04,Math.min(depth+(getCombinedOptions().handDistance||0)/100,-headBackWall())),length/observed).map(p=>new THREE.Vector3().fromArray(p).add(new THREE.Vector3(0,(getCombinedOptions().handHeight||0)/100,0)));
}
...
await rig.ready;
tips=buildTips(rig);const drivers={R:directDriver(rig,tips),L:directDriver(rig,tips)};const driveDirect=(...args)=>{args[0]=relaxedPoints(args[0],args[1],args[2],args[3]);return assistHeadContact(args[1],keepHandBeforeWall(args[1],drivers[args[1]](...args)),args[4].lm);};let directResult=null;
...
notice('Alien 18.7 ready. Manual fit controls available; tracking runs on the PC.');
...
const fixtures={cleanhands:'../test/cleanhands.webm',cheek:'./references/cheek.jpg',scalp:'./references/scalp.jpg',forward:'./references/forward.jpg',seated:'../test/seated_desk.jpg',crossed:'../test/crossed_hands.jpg',gesture:'../test/gesture67.webm'};
if(fixtures[fixtureName]){
 ...
 button.onclick=async()=>{
  ...
  if(/\.webm$/.test(fixtures[fixtureName])){source=document.createElement('video');source.src=fixtures[fixtureName];source.muted=true;source.loop=true;source.playsInline=true;await source.play();cleanup=()=>source.pause();}
...
// Keep legacy inputs for internal compatibility; present only active controls.
{
 ...
 const allowed=new Set(['handsTouch','relaxedPalm','handDistance','handHeight','headSide','contactThreshold','headHeight','headSize','headBack','fingerThickness','tipInset','lockBody','turnGain','moveGain','neckShare','trackingGrace','faceGrace','showHead','mappedFaceDots','fingerNoise','directionSmoothing','confirmJump','depthSmooth','headSmooth']);
...
const pipelineInfo=document.createElement('p');pipelineInfo.textContent='Depth follows palm size. When tracked hand-contact points stay close, optional assistance aligns their relative depth. Both hands keep fixed proportions and the head back-wall limit.';$('directSettings').prepend(pipelineInfo);
