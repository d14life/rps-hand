import {drawFace,drawShoulders,encodeAux,decodeAux} from './face-overlay.mjs?v=demo1';
import {startTracking,defaults} from './tracking-session.mjs?v=demo1';
...
export function receivePhone(onResult,onStatus,onEnd,onAux=()=>{},getOptions=()=>defaults,onStats=()=>{}){
 ...
 const url=new URL('camera.html',import.meta.url);url.searchParams.set('pair',id);url.searchParams.set('v','20.2');
...
export async function sendPhone(id,video,status,facing='user'){
 ...
 try{
  link=await connectLink(id,{onStatus:status,subscribeTo:'c',onMessage:(kind,payload)=>{if(kind==='c'){try{const msg=JSON.parse(payloadText(payload));if(msg.settings){receivedSettings=true;for(const k of Object.keys(defaults)){const v=msg.settings[k];if(['handPriority','uncappedTracking'].includes(k)&&typeof v==='boolean')settings[k]=v;else if(k==='trackerDelegate'&&['GPU','CPU'].includes(v))settings[k]=v;else if(Number.isFinite(v))settings[k]=Math.max(k==='trackingWidth'?192:0,Math.min(k==='trackingWidth'?640:60,v));}}}catch{}link?.sendJSON('p',{stage,w:video.videoWidth||0,h:video.videoHeight||0});}}});
