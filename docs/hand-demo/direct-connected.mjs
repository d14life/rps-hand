...
function solve(now){
 ...
 poseHead(rig,{pitch:e.x,yaw:-e.y,roll:-e.z});rig.root.updateMatrixWorld(true);pinTrackedEyes(rig,eyePosition);rig.root.updateMatrixWorld(true);head.update();
...
$('savePng').onclick=()=>{const a=document.createElement('a');a.href=renderer.domElement.toDataURL();a.download='connected-view.png';a.click();};notice('Demo 12: direct arm directions; forward-only limits; no contact assistance or smoothing. Fixed dimensions.');
