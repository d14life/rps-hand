import {straightenReference} from './straight-reference.mjs?v=straight13';
...
export function frontReference(hand,aspect=1){
 ...
 for(const base of [1,5,9,13,17]){const segments=[0,1,2].map(k=>sub(w[base+k+1],w[base+k]));if(length(sub(w[base+3],w[base]))/segments.reduce((s,v)=>s+length(v),0)<.92||segments.reduce((s,v)=>s+length(v),0)<1e-8)return {error:'Straighten all fingers, including the thumb and spread them comfortably.'};}
...
export class FrontCapture{
 reset(){this.samples=[];this.directions=[];this.start=null;this.time=null;this.label=null;}
 ...
 this.start??=now;this.label=hand.label;this.time=hand.time;this.samples.push(ref.points);if(hand.neutralDirections)this.directions.push(hand.neutralDirections);
 const progress=Math.min(1,(now-this.start)/1500);if(progress<1||this.samples.length<12)return {progress};
 const points=ref.points.map((_,i)=>[0,1].map(k=>this.samples.reduce((sum,s)=>sum+s[i][k],0)/this.samples.length));const neutralDirections=this.directions.length===this.samples.length?this.directions[0].map((_,i)=>[0,1,2].map(k=>this.directions.reduce((sum,d)=>sum+d[i][k],0)/this.directions.length)):null;return {progress:1,points:straightenReference(points),neutralDirections};
 }
}
