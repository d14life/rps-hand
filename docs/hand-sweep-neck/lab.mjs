...
import {startTracking as startSweepTracking,defaults as trackingDefaults} from './tracking-session.mjs?v=ios-start2';
...
function ensureWorker(){if(workerReady)return workerReady;worker=new Worker(new URL('./tracker.mjs?v=ios-start2&task=hands&delegate=GPU',import.meta.url),{type:'module'});
...
async function detectAuxFrame(frame,time,task){
 const bitmap=await createImageBitmap(frame),w=new Worker(new URL('./tracker.mjs?v=ios-start2&task='+task+'&delegate=GPU',import.meta.url),{type:'module'});
 let sent=false;
...
function showTrackingStats(s){if(stream){const errors=Object.entries(s.errors||{}),loading=Object.entries(s.loading||{});$('captureState').textContent=errors.length?'Camera connected · tracking error: '+errors.map(([k,v])=>k+': '+v).join(' · '):loading.length?'Camera connected · '+loading.map(([k,v])=>k+': '+v).join(' · '):'Camera connected · tracking active';}performanceOptions?.sample(s,measuredScene,parseFloat($('calculationTiming').textContent.split(':')[1])||0);trackingStats=s;lastStatsTime=performance.now();updateLiveHud(lastStatsTime);$('liveFps').textContent=`CAM ${s.camera||0} FPS
...
Hands: ${s.hands||0} FPS · ${s.loading?.hands||s.delegate?.hands||'waiting'} · ${Math.round(s.ms?.hands||0)} ms
Face/head: ${s.face||0} FPS · ${s.loading?.face||s.delegate?.face||'waiting/off'} · shoulders: ${s.pose||0} FPS · ${s.loading?.pose||s.delegate?.pose||'waiting/off'}
