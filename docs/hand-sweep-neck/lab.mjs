import {videoSession} from '../shared-phone/video-session.mjs?v=handoff1';
import {installPerformance} from './performance-options.mjs?v=constraints3';
let performanceOptions=null;
import {openCamera} from '../shared-phone/camera-capture.mjs?v=camera-restore1';
...
import {startTracking as startSweepTracking,defaults as trackingDefaults} from './tracking-session.mjs?v=handoff1';
...
import {receivePhone} from './video-link.mjs?v=camera-restore1';
...
function ensureWorker(){if(workerReady)return workerReady;worker=new Worker(new URL('./tracker.mjs?v=camera-restore1&task=hands&delegate=GPU',import.meta.url),{type:'module'});
...
async function detectAuxFrame(frame,time,task){
 const bitmap=await createImageBitmap(frame),w=new Worker(new URL('./tracker.mjs?v=camera-restore1&task='+task+'&delegate=GPU',import.meta.url),{type:'module'});
 let sent=false;
...
const phoneVideoSession=videoSession({video:$('video'),start:()=>{const token=epoch;liveSession=startTracking($('video'),(data,frame)=>{if(token!==epoch)return;if(data.task==='hands')acceptTracking(data,frame,token);else{combined.receive(data);drawPreview(frame,latest?.landmarks);}},showTrackingStats,getCombinedOptions);return ()=>{liveSession?.();liveSession=null;};},onConnected:message=>{$('captureState').textContent='iPhone '+message;notice('iPhone video connected. Tracking runs on this computer.');updateMode();},onError:e=>{$('captureState').textContent='iPhone video received; tracking could not start: '+e.message;notice($('captureState').textContent);}});
$('phone').onclick=()=>{stopCamera();editing=false;sampleSource=null;latest=null;updateMode();const token=epoch;$('captureState').textContent='Waiting for iPhone video. Keep its camera page open, or scan the QR and tap Start camera.';try{closePhone=receivePhone(received=>{if(token!==epoch){received.getTracks().forEach(t=>t.stop());return;}stream=received;phoneActive=true;phoneVideoSession.attach(received);},message=>{notice(message);if(token===epoch&&!stream)$('captureState').textContent=message;},message=>{stopCamera();notice(message);});}catch(e){$('captureState').textContent='Phone connection could not start: '+e.message;notice(e.message);}};
...
function updateLiveHud(now){
 ...
 liveHud.textContent=(s?(new URLSearchParams(location.search).has('fixture')?'TEST INPUT':Object.keys(s.errors||{}).length?'VIDEO CONNECTED · TRACKING ERROR':s.hands||s.face||s.pose?'LIVE TRACKING':'VIDEO CONNECTED · TRACKING STARTING'):(stream?'VIDEO CONNECTED · WAITING FOR TRACKING':'CAMERA OFF'))+'\nCamera '+number(s?.camera)+' FPS · Hands '+number(s?.hands)+' FPS · Scene '+number(measuredScene)+' FPS\nFace '+number(s?.face)+' · Shoulders '+number(s?.pose)+' FPS\nHand inference '+number(s?.ms?.hands)+' ms · Frame-to-result '+number(s?.age?.hands)+' ms';
}
...
function showTrackingStats(s){if(stream){const errors=Object.entries(s.errors||{}),active=s.hands>0||s.face>0||s.pose>0;const prefix=phoneActive?'iPhone video connected':'Camera connected';if(errors.length)$('captureState').textContent=prefix+' · tracking error: '+errors.map(([k,v])=>k+': '+v).join(' · ');else if(!active)$('captureState').textContent=prefix+' · '+Object.entries(s.loading||{}).map(([k,v])=>k+': '+v).join(' · ')+' · waiting for tracking results';}performanceOptions?.sample(s,measuredScene,parseFloat($('calculationTiming').textContent.split(':')[1])||0);trackingStats=s;lastStatsTime=performance.now();updateLiveHud(lastStatsTime);$('liveFps').textContent=`CAM ${s.camera||0} FPS
...
{
 const b=document.createElement('button');b.id='restartTracking';b.textContent='Restart tracking · keep camera';b.style.cssText='display:block!important';$('phone').after(b);
 b.onclick=()=>{if(!stream){notice('Connect your camera first.');return;}if(phoneActive){phoneVideoSession.restart();return;}liveSession?.();const token=epoch;liveSession=startTracking($('video'),(data,frame)=>{if(token!==epoch)return;if(data.task==='hands')acceptTracking(data,frame,token);else{combined.receive(data);drawPreview(frame,latest?.landmarks);}},showTrackingStats,getCombinedOptions);$('captureState').textContent='Camera connected · tracking restarting';};
}
