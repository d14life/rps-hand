import {neutralDirections,applyNeutral} from './straight-reference.mjs?v=straight13';
let capturedNeutral=null,neutralTargets=null;const neutralInputs={};
let mapLab=null;
...
import {installExperiment} from './experiment.mjs?v=touch2.4.18-final';
...
import {CombinedHead} from './head-model.mjs?v=touch2.4.18-final';
...
await rig.ready;const {snapshotModel,installSweepSettings}=await import('./settings.mjs?v=straight13');const restoreModel=snapshotModel(rig);const photoReport=fitPhotoHand(rig);
tips=buildTips(rig);for(const S of ['R','L'])for(const [f,name]of ['Thumb','Index','Middle','Ring','Pinky'].entries())tips[S+name].copy(rig.photoReference.targets[S][4+4*f]).sub(rig.photoReference.targets[S][3+4*f]);const drivers={R:directDriver(rig,tips),L:directDriver(rig,tips)};const driveDirect=(...args)=>{resetHandScale(rig,args[1]);args[0]=placePalmBeforeFingers(relaxedPoints(args[0],args[1],args[2],args[3]),args[1],args[2]);const S=args[1];if(!document.body.dataset.mapLab){try{neutralInputs[S]={hand:allHands.find(h=>h.label===(S==='L'?'Left':'Right')),directions:neutralDirections(args[0].map(p=>p.toArray()),S)};if(capturedNeutral)args[0]=applyNeutral(args[0].map(p=>p.toArray()),S,capturedNeutral,neutralTargets[S]).map(p=>new THREE.Vector3().fromArray(p));}catch{delete neutralInputs[S];}}let result=drivers[S](...args);if(mapLab)result=mapLab.drive(args[1],args[2],result,args[3]);applyHandScale(rig,args[1],result,depthExperiment?.recording?1:depthExperiment?.fit?.handScale??1);mapLab?.scale(args[1],depthExperiment?.recording?1:depthExperiment?.fit?.handScale??1);applyDisplayOffset(args[1],result);return keepHandBeforeWall(args[1],result);};let directResult=null;
...
if(!document.body.dataset.mapLab)installSweepSettings({rig,hands:()=>Object.values(neutralInputs).filter(v=>v.hand).map(v=>({...v.hand,neutralDirections:v.directions})),aspect:()=>captureAspect,applyModel:(referencePoints,neutral)=>{
 restoreModel();fitPhotoHand(rig,{referencePoints});capturedNeutral=neutral||null;neutralTargets=Object.fromEntries(['R','L'].map(S=>[S,neutralDirections(rig.photoReference.targets[S].map(p=>p.toArray()),S)]));for(const key of Object.keys(basisCache))delete basisCache[key];
