...
self.onmessage=async({data})=>{try{
 if(data.type==='init'){
  ...
  const source=data.bitmap||data.image,start=performance.now();try{
   ...
   // Forward eye direction to the head rig, without sending other facial expressions.
   if(task==='face')out.face=r.faceLandmarks?.[0]&&r.facialTransformationMatrixes?.[0]?{matrix:Array.from(r.facialTransformationMatrixes[0].data),points:Object.fromEntries(faceIds.map(i=>[i,{x:round(r.faceLandmarks[0][i].x),y:round(r.faceLandmarks[0][i].y)}])),eyes:Object.fromEntries((r.faceBlendshapes?.[0]?.categories||[]).filter(c=>c.categoryName?.startsWith('eyeLook')).map(c=>[c.categoryName,c.score]))}:null;
