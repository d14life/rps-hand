...
export async function connectLink(code, { onMessage = () => {}, onStatus = () => {}, subscribe = true, subscribeTo = "#" } = {}) {
  ...
  const clients = new Array(BROKERS.length).fill(null);const pending=[];
  ...
  try{await ready;}catch(e){dead=true;for(const c of pending)c.end(true);throw e;}
  return {
    code, base,
    ...
    close() { dead = true; for (const c of pending) try { c?.end(true); } catch {} },
  };
}
