...
const validCode=code=>/^[a-f0-9]{32}$/.test(code);
...
export function receivePhone(onResult,onStatus,onEnd,onAux=()=>{},getOptions=()=>defaults,onStats=()=>{}){
 ...
 let id;try{id=localStorage.getItem('rps-relay-pair-v2');if(!validCode(id)){id=newCode();localStorage.setItem('rps-relay-pair-v2',id);}}catch{id=newCode();}const owner=crypto.randomUUID(),channel=new BroadcastChannel('rps-relay-owner-v2');let link=null,closed=false,connected=false,heartbeat=null,size={w:640,h:480},trackerDetail='';
 const box=document.createElement('dialog');box.className='phonePair';
 box.innerHTML='<h2>Use your phone camera</h2><p>Scan this QR on the other phone, then tap <b>Start camera</b>.<br>Mobile-data fallback: the phone tracks locally. Camera images stay on the phone; coordinates pass through public relay servers.</p><div class="qr"></div><p class="code"></p><p><a target="_blank" rel="noopener">Open phone camera page</a></p><div class="row"><button type="button" data-copy>Copy phone link</button><button type="button" data-cancel>Cancel</button></div><p class="pairStatus" role="status">Connecting PC to relay…</p>';
 const url=new URL('camera-relay.html',import.meta.url);url.searchParams.set('pair',id);url.searchParams.set('v','network1');if(getOptions().fullBody)url.searchParams.set('body','1');if(getOptions().fullBody&&getOptions().handRate>0)url.searchParams.set('connected','1');
 ...
 const close=()=>{if(closed)return;closed=true;clearInterval(heartbeat);link?.close();channel.close();if(box.open)box.close();box.remove();};
 const cancel=()=>{close();onEnd('Phone connection cancelled');};channel.onmessage=e=>{if(e.data?.owner!==owner){close();onEnd('Relay camera moved to another version.');}};channel.postMessage({owner});
...
export async function sendPhone(id,video,status,facing='user'){
 ...
 if(new URLSearchParams(location.search).get('body')==='1')settings={...settings,fullBody:true,handRate:new URLSearchParams(location.search).get('connected')==='1'?30:0,faceRate:new URLSearchParams(location.search).get('connected')==='1'?30:0};
 ...
 try{
  link=await connectLink(id,{onStatus:status,subscribeTo:'c',onMessage:(kind,payload)=>{if(kind==='c'){try{const msg=JSON.parse(payloadText(payload));if(msg.settings){receivedSettings=true;for(const k of Object.keys(defaults)){const v=msg.settings[k];if(['handPriority','uncappedTracking','fullBody'].includes(k)&&typeof v==='boolean')settings[k]=v;else if(k==='poseModel'&&['lite','full','heavy'].includes(v))settings[k]=v;else if(k==='trackerDelegate'&&['GPU','CPU'].includes(v))settings[k]=v;else if(Number.isFinite(v))settings[k]=Math.max(k==='trackingWidth'?192:0,Math.min(k==='trackingWidth'?640:60,v));}}}catch{}link?.sendJSON('p',{stage,w:video.videoWidth||0,h:video.videoHeight||0});}}});
