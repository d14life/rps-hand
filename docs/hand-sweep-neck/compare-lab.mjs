...
import {installCombinedUI} from '../hand-compare/combined-hand-sweep-neck.mjs';
...
await rig.ready;const {snapshotModel,installSweepSettings}=await import('../hand-compare/settings.mjs');const restoreModel=snapshotModel(rig);const photoReport=variant==='archive'?null:fitPhotoHand(rig);
tips=buildTips(rig);if(rig.photoReference)for(const S of ['R','L'])for(const [f,name]of ['Thumb','Index','Middle','Ring','Pinky'].entries())tips[S+name].copy(rig.photoReference.targets[S][4+4*f]).sub(rig.photoReference.targets[S][3+4*f]);const drivers={R:directDriver(rig,tips),L:directDriver(rig,tips)};const driveDirect=(...args)=>{resetHandScale(rig,args[1]);args[0]=placePalmBeforeFingers(relaxedPoints(args[0],args[1],args[2],args[3]),args[1],args[2]);let result=drivers[args[1]](...args);if(mapLab)result=mapLab.drive(args[1],args[2],result,args[3]);applyHandScale(rig,args[1],result,depthExperiment?.recording?1:depthExperiment?.fit?.handScale??1);mapLab?.scale(args[1],depthExperiment?.recording?1:depthExperiment?.fit?.handScale??1);applyDisplayOffset(args[1],result);return keepHandBeforeWall(args[1],result);};let directResult=null;
...
try{const saved=JSON.parse(localStorage.getItem('compare-hand-sweep-neck-'+variant+'-hand-sweep-neck-start')||'null');if(saved)restoreStart(saved);}catch{}
$('saveStart').onclick=()=>{try{localStorage.setItem('compare-hand-sweep-neck-'+variant+'-hand-sweep-neck-start',JSON.stringify({...Object.fromEntries(startFields.map(id=>[id,+$(id).value])),...Object.fromEntries(booleanStartFields.map(id=>[id,$(id).checked]))}));$('startStatus').textContent='Starting settings saved in this browser.';}catch{$('startStatus').textContent='Browser storage unavailable.';}};
...
if(!document.body.dataset.mapLab)installSweepSettings({cameraPanel:firstPerson.panel,rig,hands:()=>allHands,aspect:()=>captureAspect,applyModel:referencePoints=>{
 ...
 tips=buildTips(rig);if(rig.photoReference)for(const S of ['R','L'])for(const [f,name]of FINGERS.entries())tips[S+name].copy(rig.photoReference.targets[S][4+4*f]).sub(rig.photoReference.targets[S][3+4*f]);
