...
export function installTwoHandMagnets({container,rig,hands,rendered,aspect,config,isPaused}){
 ...
 function update(now){
  ...
  if(!pairs.length){status.textContent=cfg.enabled?'Bring fingertips on opposite hands together.':'Two-hand fingertip magnets off.';return;}
  ...
  for(let pass=0;pass<40;pass++){
   for(const pair of pairs){const l=pair.a/4-1,r=pair.b/4-1,target=chains.L[l][3].clone().add(chains.R[r][3]).multiplyScalar(.5);reach(chains.L[l],lengths.L[l],target);reach(chains.R[r],lengths.R[r],target);}
